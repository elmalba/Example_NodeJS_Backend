...
test('bar', function (t) {
    ...
    resolve('foo', { basedir: dir + '/bar' }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar/node_modules/foo/iniciar.js'));
...
test('biz', function (t) {
    ...
    resolve('./grux', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'grux/iniciar.js'));
        t.equal(pkg, undefined);
    });
    ...
    resolve('./garply', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'garply/lib/iniciar.js'));
    ...
    resolve('tiv', { basedir: dir + '/grux' }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'tiv/iniciar.js'));
        t.equal(pkg, undefined);
    });
...
test('quux', function (t) {
    ...
    resolve('./foo', { basedir: dir, 'package': { main: 'quux' } }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'foo/iniciar.js'));
...
test('#25: node modules with the same name as node stdlib modules', function (t) {
    ...
    resolve('punycode', { basedir: resolverDir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(resolverDir, 'node_modules/punycode/iniciar.js'));
    });
});
...
test('#52 - incorrectly resolves module-paths like "./someFolder/" when there is a file of the same name', function (t) {
    ...
    resolve('./foo/', { basedir: path.join(dir, 'same_names') }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'same_names/foo/iniciar.js'));
    });
});
...
test('async dot main', function (t) {
    ...
    resolve('./resolver/dot_main', function (err, ret) {
        t.notOk(err);
        t.equal(ret, path.join(__dirname, 'resolver/dot_main/iniciar.js'));
...
test('async dot slash main', function (t) {
    ...
    resolve('./resolver/dot_slash_main', function (err, ret) {
        t.notOk(err);
        t.equal(ret, path.join(__dirname, 'resolver/dot_slash_main/iniciar.js'));
        t.ok(new Date() - start < 50, 'resolve.sync timedout');
        t.end();
    });
});
