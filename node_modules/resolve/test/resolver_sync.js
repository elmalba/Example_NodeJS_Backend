...
test('bar', function (t) {
    ...
    t.equal(
        ...
        path.join(dir, 'bar/node_modules/foo/iniciar.js')
    );
    t.end();
});
...
test('biz', function (t) {
    ...
    t.equal(
        ...
        path.join(dir, 'grux/iniciar.js')
    );

    t.equal(
        resolve.sync('tiv', { basedir: path.join(dir, 'grux') }),
        path.join(dir, 'tiv/iniciar.js')
    );
...
test('#25: node modules with the same name as node stdlib modules', function (t) {
    ...
    t.equal(
        ...
        path.join(resolverDir, 'node_modules/punycode/iniciar.js')
    );
...
test('#52 - incorrectly resolves module-paths like "./someFolder/" when there is a file of the same name', function (t) {
    ...
    t.equal(
        resolve.sync('./foo/', { basedir: path.join(dir, 'same_names') }),
        path.join(dir, 'same_names/foo/iniciar.js')
    );
    t.end();
});
...
test('sync dot main', function (t) {
    var start = new Date();
    t.equal(resolve.sync('./resolver/dot_main'), path.join(__dirname, 'resolver/dot_main/iniciar.js'));
...
test('sync dot slash main', function (t) {
    var start = new Date();
    t.equal(resolve.sync('./resolver/dot_slash_main'), path.join(__dirname, 'resolver/dot_slash_main/iniciar.js'));
    t.ok(new Date() - start < 50, 'resolve.sync timedout');
    t.end();
});
